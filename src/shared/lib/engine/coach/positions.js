...
export async function fetchPuzzles() {
  ...
  fetchPromise = (async () => {
    try {
      const resp = await fetch('/puzzle_db/tactics.csv');
      if (!resp.ok) return [];
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (parts.length >= 3) {
          ...
          const initialFen = parts[1];
          const solutionStr = parts[2];
          if (initialFen && solutionStr) {
            puzzles.push({
              puzzleId,
              initialFen,
              tacticalSolution: solutionStr.trim().split(/\s+/),
            });
          }
        }
      }
      ...
      console.warn('Failed to load tactics.csv, using fallback positions:', err);
      return [];
    }
  })();
...
export async function getRandomPuzzle() {
  ...
  const fallbackFen = pickRandomPosition();
  return {
    puzzleId: 'fallback',
    initialFen: fallbackFen,
    tacticalSolution: [],
  };
}
