...
const PLAN_PLIES = 5; // walk the principal variation up to this many plies (max 5 plies)
...
function formatVerdictString(evalCp, mate = null) {
...
export async function buildFullExplanation(fen, opts = {}) {
  ...
  // Key strategic squares: outposts, weak pawn targets, or key tactical target squares in plan
  const keySquareSet = new Set();

  if (staticBlob?.activity) {
    const outposts = [
      ...(staticBlob.activity.white?.outposts || []),
      ...(staticBlob.activity.black?.outposts || []),
      ...(staticBlob.activity.outposts || []),
    ];
    outposts.forEach(o => {
      const sq = typeof o === 'string' ? o : o?.square;
      if (sq) keySquareSet.add(sq);
    });
  }

  planSteps.forEach(step => {
    if (step.raw_motifs) {
      step.raw_motifs.forEach(m => {
        if (['outpost', 'knight_invasion', 'pawn_breakthrough', 'fork', 'pin', 'skewer', 'overloaded'].includes(m.id)) {
          (m.targets || []).forEach(sq => keySquareSet.add(sq));
        }
      });
    }
  });

  const keySquares = [...keySquareSet].sort();
  ...
  if (typeof engineRes.score === 'number') {
    ...
    const matePov = (engineRes.mate !== null && engineRes.mate !== undefined)
      ? (stmIsWhite_ ? engineRes.mate : -engineRes.mate)
      : null;
    staticBlob.static_eval_cp = staticBlob.eval_cp;
    staticBlob.eval_cp = whitePovScore;
    staticBlob.eval_pawns = whitePovScore / 100;
    staticBlob.eval_mate = matePov;
    staticBlob.verdict = formatVerdictString(whitePovScore, matePov);
  }
  ...
  const userMultiPV = opts.multipv || defaults.multipv;
  staticBlob.engine_top_moves = annotatedMoves.slice(0, userMultiPV);
