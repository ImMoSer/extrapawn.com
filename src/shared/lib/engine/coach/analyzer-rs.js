...
export function composeTagline(rustResult) {
  ...
  if (has('decisive_combination'))
    return out(rustResult, motifIds, phraseFor('decisive_combination'))
  ...
  if (has('anastasia_mate_threat'))
    return out(rustResult, motifIds, "Anastasia's mate threat (knight cut-off + rook on the rim)")
  if (has('bodens_mate_threat'))
    return out(rustResult, motifIds, "Boden's mate threat (two bishops crossfire)")
  if (has('arabian_mate_threat'))
    return out(
      rustResult,
      motifIds,
      'Arabian-style mate threat (rook + knight on the cornered king)',
    )
  ...
  if (has('removes_defender') && has('threatens')) {
    return out(rustResult, motifIds, `${phraseFor('removes_defender')}, leaving it undefended`)
  }
  ...
  if (has('pin') && has('threatens')) {
    return out(rustResult, motifIds, `${phraseFor('pin')}, threatening to win it`)
  }
  if (has('rook_lift') && has('eyes_king_zone')) {
    return out(rustResult, motifIds, `${phraseFor('rook_lift')} — joining the king attack`)
  }
  if (has('opens_file_for') && has('battery')) {
    return out(rustResult, motifIds, phraseFor('battery'))
  }
  if (has('simplifies') && has('check')) {
    return out(rustResult, motifIds, `${phraseFor('simplifies')} with check`)
  }
  if (has('promotion') && has('check')) {
    return out(rustResult, motifIds, `${phraseFor('promotion')} with check`)
  }
  if (has('promotion') && has('checkmate')) {
    return out(rustResult, motifIds, `${phraseFor('promotion')} — mate`)
  }
