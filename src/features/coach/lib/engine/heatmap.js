...
import { getPieces, removePiece, getSideToMove, getLegalDestinations, makeMove } from './chess'
import { isReady as wasmReady, pieceContributionsForFen, pieceValueAt } from './analyzer-rs.js'
...
function ownerValue(evalRes, sideToMoveAtFen, ownerColor) {
  ...
  const whiteMate = mate !== null ? (sideToMoveAtFen === 'w' ? mate : -mate) : null
  // Then to owner POV.
  const ownerCp = ownerColor === 'w' ? whiteCp : -whiteCp
  const ownerMate = whiteMate !== null ? (ownerColor === 'w' ? whiteMate : -whiteMate) : null
  if (ownerMate !== null) {
    ...
    for (const dest of dests) {
      ...
      try {
        ...
        const valueCp = clampDelta(
          ownerValue(baseRes, newTurn, piece.color) - ownerValue(withoutRes, newTurn, piece.color),
        )
        onResult({
          dest,
          value_cp: valueCp,
