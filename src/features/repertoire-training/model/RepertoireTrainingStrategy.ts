...
export class RepertoireTrainingStrategy implements IGameplayStrategy {
  ...
  private trainingStore = useRepertoireTrainingStore()
  ...
  async validateUserMove(uciMove: string): Promise<boolean> {
    if (!this.trainingStore.isTrainingActive) return false
  ...
  async requestBotMove(): Promise<string | null> {
    if (!this.trainingStore.isTrainingActive) return null
  ...
  private onVariationEnded() {
    ...
    this.studyStore.persistNodeMetadata(nodePath, {
      successes: training.successes,
      attempts: training.attempts,
      lastTrained: training.lastTrained,
